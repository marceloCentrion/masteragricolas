                ...
                <div class="col-7">
                  <NuxtLink to="">
                    <div style="text-align: right">
                      <NuxtLink to="/admin/categorias/cadastrar-categoria">
                        <button type="button" class="btn btn-outline-light">Nova Categoria</button>
                      </NuxtLink>
                    </div>
                  </NuxtLink>
                </div>
                ...
                <table class="table table-hover">
                  ...
                  <tbody>
                    <tr v-for="cat in state.categorias" :key="cat.id">
                      ...
                      <td>{{ cat.urn }}</td>
                      <td>
                        <NuxtLink :to="'/admin/categorias/' + cat.id">
    ...
    async function deletarCategoria(id) {
      if (confirm("Deseja deletar esta categoria?")) {
        try {
          ...
          const { data } = await services.categoria.delete({
            id: id,
            token,
          });
